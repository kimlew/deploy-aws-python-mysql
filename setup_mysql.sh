...
# - installs MySQL on a machine & creates the database & table needed for the
# Python Flask app, create-jazz-lyric.
# - runs as part of main.sh
# - runs BEFORE setup_machine.sh, since setup_machine.sh depends on things
# set up here first, e.g., MYSQL_ROOT_PWD.
# So the root password for MySQL you are prompted for when you run main.sh:
# - creates a the file, mysql_pw.txt on the instance, OUTSIDE of this script &
# puts in that MySQL password, which you need BEFORE this script runs.
# - is in put the file INSTEAD OF in .env as an environment variable - so it is
# on the filesystem & can be reused for other purposes.
...
# But since this script is called from main.sh, you are passed back to main.sh,
# which ssh-es to home/ec2-user & runs setup_machine.sh which makes sure you
# start on AWS at: cd /home/ec2-user.

# DID as TEST CASE 1: Manually uninstall MySQL client, db files, MySQL server, &
